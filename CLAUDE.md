...
- Rust modules: `process_manager.rs` (spawn/stop ComfyUI, model discovery via embedded `local_model_discovery.py`), `image_gallery.rs` (output index + thumbnail cache), `model_manager.rs` (local model index, hashing, Civitai sync), `library_manager.rs`, `preset_manager.rs`, `prompt_suggestions.rs`, `download_manager.rs`, `civitai.rs`, `animadex.rs`, `danbooru_wiki.rs`, `network_cache.rs`, and `booru/` (a `Provider` trait in `mod.rs`; API engines `danbooru.rs`, `gelbooru.rs`, `moebooru.rs`; one file per site under `booru/sources/` holding a configured engine static, registered in `sources/mod.rs` — add a site by adding a `sources/<site>.rs` and appending it to `ALL`; a site with its own API implements `Provider` directly there, like `aitag.rs`).
